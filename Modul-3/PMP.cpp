...
int form = 0;
void inorder_format(AVLNode *root) {
    if (root) {
        inorder_format(root->left);
        printf("%s", root->data);
        ++form;
        if (form < cnt) printf("_");
        inorder_format(root->right);
    }
}

void call_inorder(AVL *avl) {
    inorder(avl->_root);
}

void call_inorder_format(AVL *avl) {
    inorder_format(avl->_root);
}
